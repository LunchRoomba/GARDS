...
def maskBackground(image, colorRange):
	...
	cv.floodFill(image["image"], image["mask"], (1, 1), WHITE, loDiff=(colorRange, colorRange, colorRange), upDiff=(colorRange, colorRange, colorRange), flags=4 | ( 255 << 8 ))
	return image
...
def fileIn():
	...
	for i in range(len(input)):
		...
		for i, img in enumerate(images):
			...
			if (h>MIN_PIXEL_WIDTH and h<MAX_PIXEL_WIDTH and w>MIN_PIXEL_WIDTH and h<MAX_PIXEL_WIDTH):
				#filename = "./output/{0}_{1}-{2}_{3}.jpg"
				#cv.imwrite(filename.format(title, img["centerx"], img["centery"], "image"), img["image"])
